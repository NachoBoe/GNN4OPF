...
def objective(trial):
    ...
    c_1 = trial.suggest_loguniform('c_1', 0.01, 0.2)
    ...
    layers =  trial.suggest_categorical('layers', [[3,32,32,4], [3,128,128,4], [3,512,512, 4], [3,2048,2048, 4], [3,32,32,32,4], [3,128,128,128,4], [3,512,512,512,4]])
    ...
    model = GNNUnsupervised(cfg.model.layers, edge_index, edge_weights, num_layers, cfg.model.K, min_vector, max_vector, num_nodes, batch_norm=cfg.training.batch_norm, use_edge_weights=cfg.training.use_edge_weights).to(device)
    ...
    for epoch in range(num_epochs):
        train_loss = run_epoch(model, train_loader, optimizer, criterion, Y_line, Y_bus, max_ika, dual_variables, epoch, writer, node_weights)
        val_loss = evaluate(model, val_loader, criterion, Y_line, Y_bus, max_ika, dual_variables, epoch, writer, node_weights)
