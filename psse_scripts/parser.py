...
def switched_shunt_data_df(archivo):
    ...
    # Define los nombres de las columnas
    nombres_columnas = ['I', 'MODSW', 'ADJM', 'ST', 'VSWHI', 'VSWLO', 'SWREG', 'RMPCT', 'RMIDNT', 'BINIT', 'N1', 'B1', 'N2', 'B2', 'N3', 'B3', 'N4', 'B4']
    # Crea el DataFrame
    shunt_data = pd.DataFrame(datos_shunt, columns=nombres_columnas)

    # Convierte las columnas numéricas a tipos numéricos
    columnas_numericas = ['MODSW', 'ADJM', 'ST', 'VSWHI', 'VSWLO', 'RMPCT', 'N1', 'B1', 'N2', 'B2', 'N3', 'B3', 'N4', 'B4']
    shunt_data[columnas_numericas] = shunt_data[columnas_numericas].apply(pd.to_numeric).replace(np.nan, 0)

    shunt_data['N'] = np.sign(shunt_data['B1']**2) + np.sign(shunt_data['B2']**2) + np.sign(shunt_data['B3']**2) + np.sign(shunt_data['B4']**2)
