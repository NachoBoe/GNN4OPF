...
def find_best_run(path, k=10):
    ...
    for file in os.listdir(path):
        try:
            ...
            voltage_and_feas.append((datos['voltaje_setpoint_metric']))
        except:
            pass


    indices_ordenados = sorted(range(len(voltage_and_feas)), key=lambda i: voltage_and_feas[i], reverse=False)
    ...
    k_best_voltage_and_feas = [voltage_and_feas[i] for i in best_values]

    return k_best_trains, k_best_losses, k_best_feasibilities, k_best_voltage_setpoints, k_best_voltage_and_feas
